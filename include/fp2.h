 ...
 * This function is not argument-safe, i.e. calling fp2_mul(r, x, y) for r = x or r = y will provide incorrect results.
 */
void fp2_mul_unsafe(fp2_t res, const fp2_t x, const fp2_t y);

/*
 * @brief Calculate mul: x[a + bi] *= y[a + bi]. 
 * This function is argument-safe, i.e. calling fp2_mul(r, x, y) for r = x or r = y is allowed.
 */
...
static inline void fp2_div_unsafe(fp2_t res, const fp2_t x, const fp2_t y) {
    assert(res != y && res != x && "Fp^2 division is not arg-safe, cannot be called with r = y or r = x");
