...
void pprod_set_array(pprod_t pp, unsigned int *primes, unsigned int n_primes) {
    ...
    for (unsigned int i = 0; i < n_primes; i++) {
        ...
        // Only odd primes are allowed except the first argument being power of 2 or odd number
        if (i == 0) {
            int bits = 0;
            int p = primes[i];
            while (p) { 
            ...
            assert((bits == 1 || primes[i] % 2 == 1) && "First number can only be power of 2 or odd prime");
        }
        if (i > 0) assert(primes[i] % 2 && "Only first number can be power of 2, further have to be odd primes");
