...
void msidh_state_prepare(struct msidh_state *msidh,
    ...
    assert(!fp2_equal_uint(params->a, 2) && "Curve coefficient cannot be equal to 2");
    assert(params->t >= 2 && "Security parameter t must be larger than 1");
    // assert(params->f > 0 && "MSIDH Params cofactor f must be a positive integer");
    ...
    // Do not seek the cofactor f, only calculate p = AB-f and check for primality
    int ret = msidh_calc_pub_params(msidh->p, msidh->A, msidh->B, msidh->t, msidh->f);
